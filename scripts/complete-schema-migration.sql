-- =====================================================
-- EDUGRANT GUARDIAN - COMPLETE DATABASE SCHEMA
-- Run this script in your new Supabase project's SQL Editor
-- Project URL: https://ffbdfqavzudmegqkjgra.supabase.co
-- =====================================================
...
-- Education ID auto-generation trigger
...
-- audit_logs
CREATE POLICY "super_admin_full_access_audit" ON public.audit_logs FOR ALL USING (public.has_role(auth.uid(), 'super_admin')) WITH CHECK (public.has_role(auth.uid(), 'super_admin'));
CREATE POLICY "admin_read_audit_logs" ON public.audit_logs FOR SELECT USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "chief_read_audit_logs" ON public.audit_logs FOR SELECT USING (public.has_role(auth.uid(), 'chief'));
CREATE POLICY "authenticated_insert_audit_logs" ON public.audit_logs FOR INSERT TO authenticated WITH CHECK (auth.uid() = actor_id);
...
-- invitations
CREATE POLICY "super_admin_manage_invitations" ON public.invitations FOR ALL USING (public.has_role(auth.uid(), 'super_admin')) WITH CHECK (public.has_role(auth.uid(), 'super_admin'));
CREATE POLICY "user_read_own_invite_safe" ON public.invitations FOR SELECT TO authenticated USING (invited_email = (auth.jwt() ->> 'email'::text) AND status != 'pending');
CREATE POLICY "super_admin_delete_invitations" ON public.invitations FOR DELETE TO authenticated USING (has_role(auth.uid(), 'super_admin'::app_role));

-- bursary_programs
CREATE POLICY "admins_insert_bursary_programs" ON public.bursary_programs FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY "authenticated_view_bursary_programs" ON public.bursary_programs FOR SELECT TO authenticated USING (true);
CREATE POLICY "admins_update_bursary_programs" ON public.bursary_programs FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));

-- bursary_applications
CREATE POLICY "students_insert_applications" ON public.bursary_applications FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "view_bursary_applications" ON public.bursary_applications FOR SELECT TO authenticated USING (auth.uid() = user_id OR has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY "admins_update_applications" ON public.bursary_applications FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));

-- school_payment_details
CREATE POLICY "students_manage_own_payment_details" ON public.school_payment_details FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "admins_view_payment_details" ON public.school_payment_details FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY "admins_update_payment_details" ON public.school_payment_details FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));

-- disbursements
CREATE POLICY "students_view_own_disbursements" ON public.disbursements FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM student_profiles sp WHERE sp.id = disbursements.student_id AND sp.user_id = auth.uid()));
CREATE POLICY "admins_manage_disbursements" ON public.disbursements FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));

-- admin_access_codes
CREATE POLICY "super_admin_manage_access_codes" ON public.admin_access_codes FOR ALL TO authenticated USING (has_role(auth.uid(), 'super_admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'super_admin'::app_role));

-- admin_requests
CREATE POLICY "Submit admin request with valid data" ON public.admin_requests FOR INSERT TO anon, authenticated WITH CHECK (name IS NOT NULL AND name <> '' AND email IS NOT NULL AND email <> '' AND requested_level IS NOT NULL AND requested_level <> '');
CREATE POLICY "Super admin manages requests" ON public.admin_requests FOR ALL TO authenticated USING (has_role(auth.uid(), 'super_admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY "Users view own request" ON public.admin_requests FOR SELECT TO authenticated USING (email = (auth.jwt() ->> 'email'));

-- announcements
CREATE POLICY "Authenticated view announcements" ON public.announcements FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admins create announcements" ON public.announcements FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY "Admins update own announcements" ON public.announcements FOR UPDATE TO authenticated USING ((auth.uid() = created_by AND has_role(auth.uid(), 'admin'::app_role)) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY "Super admin delete announcements" ON public.announcements FOR DELETE TO authenticated USING (has_role(auth.uid(), 'super_admin'::app_role));

-- announcement_applications
CREATE POLICY "Students apply to announcements" ON public.announcement_applications FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Students view own announcement apps" ON public.announcement_applications FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Admins manage announcement apps" ON public.announcement_applications FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role));
